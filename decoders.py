...
class WienerFilterRegression(object):
    ...
    def fit(self,X_flat_train,y_train):
        ...
        self.model=linear_model.Ridge(alpha=self.alpha) #Initialize linear regression model
...
class GRURegression(object):
    ...
    def fit(self,X_train,y_train):
        ...
        model.compile(loss='mse',optimizer='rmsprop',metrics=['accuracy']) #Set loss function and optimizer
        if keras_v1:
            model.fit(X_train,y_train,nb_epoch=self.num_epochs,verbose=self.verbose, batch_size=128) #Fit the model
        else:
            model.fit(X_train,y_train,epochs=self.num_epochs,verbose=self.verbose, batch_size=128) #Fit the model
        self.model=model
