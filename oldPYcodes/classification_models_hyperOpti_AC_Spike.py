...
variable=['New_Raster','behave_data','targetT','neuron_lab','semiToneDiff']
...
Firerate=[]
SmthWind= 25 #taftTar+tbefTar #ms
for ii in range(Raster_cut.shape[0]):
    for jj in range(Raster_cut.shape[1]-SmthWind+1):
        Firerate.append(np.sum(Raster_cut[ii,jj:jj+SmthWind])/(SmthWind/1000))
Firerate=np.array(Firerate)
...
uniNerLab=np.unique(np.squeeze(AllneuronLab))
...
#     # ind_temp=np.nonzero(np.isin((combLabs,uniNerLab[kk]))[0] 
...
AllXX=Firerate #Zscore_Firerate
...
if NumNeu==len(uniNerLab):
    ...
    ax1.legend()
    plt.xticks(x,models)
    fig1.savefig('AllAccuracy_'+filename+'_'+variable[0]+'_NumNeu'+str(NumNeu)+figNaStr+'.png')
    # plt.show()
    ...
    fig2.savefig('API_All_'+filename+'_'+variable[0]+'_NumNeu'+str(NumNeu)+figNaStr+'.png')
    # plt.show()
    ...
    for u in range(len(models)):
        ...
        plt.ylabel('True') 
    fig3.suptitle('Confusion matrix of all Classifiers'+'(Num of neurCombs = '+str(len(NerLab))+')')
    fig3.savefig('ConfMat_'+filename+'_'+variable[0]+'_NumNeu'+str(NumNeu)+figNaStr+'.png')
    # plt.show()
else:
    fig1=plt.figure(1)
    ax1=plt.gca()
